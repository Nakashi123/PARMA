...
if uploaded_file:
    try:
        ...
        score_columns = [c for c in df.columns if str(c).startswith("6_")]
        ...
        for key, idxs in perma_indices.items():
            valid = [scores[i] for i in idxs if not np.isnan(scores[i])]
            results[key] = float(np.mean(valid)) if valid else 0.0
...
<div class="section-title"><h3>各要素の説明（5つ）</h3></div>
        ...
        balance_comment = (
            "全体としてバランスよく整っています。" if std_score < 1.0 else
            "おおむね良好ですが、いくつか強弱があります。" if std_score < 2.0
            else "要素間の強弱が比較的大きい状態です。"
        )
        ...
        with st.expander("注意事項"):
            st.markdown(
                "- 本結果は“良い/悪い”ではなく**選好と環境**の反映として扱い、自分の生活史・価値観に照らして解釈しましょう。\n"
                "- 活動を新たに取り入れる時は、まず日課化しやすい**最小行動**から無理なく実施しましょう。（例：1日5分の散歩/3行だけ日記を書いてみる　など）。\n"
                "- 本ツールは**スクリーニング**であり医療的診断ではありません。心身の不調が続く場合は医療機関でのご受診を検討してください。"
            )
